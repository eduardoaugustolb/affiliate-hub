...
- **Dado pessoal sensível** (art. 5º, II): origem racial/étnica, convicção
  ...
  **O projeto não trata, hoje, nenhuma categoria sensível** — ver
  [[#Ponto de atenção — MediaTemplate e direito de imagem]] pra um caso
  limítrofe.
...
| [[03-Modules/LinkRedirect]] | Nenhum hoje (`click_logs` só tem `product_id` + `clicked_at`); planejado ganhar "métrificadores" (analytics/pixel) | tabela `click_logs` | — (se vier cookie/pixel de terceiro: consentimento, art. 7º, I) | OK hoje. Ver [[02-Decisions/ADR-0014-cookies-e-rastreamento-de-clique\|ADR-0014]] antes de adicionar qualquer tracking |
| [[03-Modules/Broadcast]] | Nenhum dado de terceiro persistido no banco do projeto (grupo de WhatsApp é gerido pelo próprio WhatsApp; `authState` do Baileys é credencial do número do bot, não dado de cliente) | `SessionStorage` (credencial do bot) | — | OK, ver [[#Ponto de atenção — Broadcast e grupo de WhatsApp]] |
| [[03-Modules/CommentAssist]] | Potencialmente identificador de usuário do TikTok (manual, não persistido em banco pelo módulo) | Nenhuma tabela — fluxo manual | — | Baixo risco, ver nota no módulo |
| [[03-Modules/AffiliateSync]] | Nenhum dado de cliente hoje (integração é com API de afiliado, não com dado de comprador) | — | — | Revisar quando o módulo for de fato implementado — ver [[#Ponto de atenção — AffiliateSync]] |
| [[03-Modules/MediaTemplate]] | Foto "lifestyle" de terceiro pode conter imagem que identifica pessoa | `FileStorage` (Cloudflare R2) | Curadoria humana obrigatória (já é regra do módulo) | Ver [[#Ponto de atenção — MediaTemplate e direito de imagem]] |
| [[03-Modules/Catalog]] | Nenhum | — | — | OK |
